...
public class AssetService {
	...
	public List<AssetCategory> getAssetCategories() {
		String query = "SELECT id,code,name FROM egasset_asset_category";
...
/*	public List<Asset> getAssetList(List<Integer> assetIdList) {

		List<Asset> list = null;

		StringBuilder query = new StringBuilder(" where asset.id in (");

		query.append(assetIdList);

		for (int i = 1; i < assetIdList.size(); i++) {

			query.append("," + assetIdList.get(i));
		}
		query.append(")");

		list = jdbcTemplate.query(query.toString(), new AssetRowMapper());
